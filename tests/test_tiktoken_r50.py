...
def test_linear_english_text_with_contractions():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH), int)
    assert estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH) == pytest.approx(12, abs=1)
...
def test_linear_english_text_empty_and_whitespace():
    message1 = ""
    assert isinstance(estimate_text_tokens(message1, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH), int)
    assert estimate_text_tokens(message1, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH) == pytest.approx(0, abs=1)
...
def test_linear_english_text_with_long_word():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH), int)
    assert estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH) == pytest.approx(15, abs=1)
...
def test_linear_english_text_with_rare_character():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH), int)
    assert estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ENGLISH) == pytest.approx(18, abs=5)
...
def test_linear_all_text_empty_and_whitespace():
    message1 = ""
    assert isinstance(estimate_text_tokens(message1, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL), int)
    assert estimate_text_tokens(message1, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL) == pytest.approx(0, abs=1)
...
def test_linear_all_text_non_english():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL), int)
    assert estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL) == pytest.approx(31, abs=26)

def test_linear_all_text_with_long_word():
    message = "This is a verylongwordwithoutspaces and should be counted properly."
    assert isinstance(estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL), int)
    assert estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL) == pytest.approx(15, abs=1)

def test_linear_all_text_with_rare_character():
    message = "What is the smallest possible value for P[A ∩ B ∩ C]?" # https://huggingface.co/datasets/allenai/WildChat-1M?conversation-viewer=18
    assert isinstance(estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL), int)
    assert estimate_text_tokens(message, TextEstimator.TIKTOKEN_R50K.LINEAR_ALL) == pytest.approx(18, abs=5)
