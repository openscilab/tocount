...
def test_universal_text_with_simple_prompt():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(6, abs=5)
...
def test_universal_text_with_contractions():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(12, abs=10)
...
def test_universal_text_with_prefixes_and_suffixes(): # https://huggingface.co/datasets/lmsys/lmsys-chat-1m?conversation-viewer=10
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(8, abs=24)
...
def test_universal_code_with_keywords():
    ...
    assert isinstance(estimate_text_tokens(message1, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message1, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(9, abs=5)

    message2 = "class QueryPlanner:" # http://huggingface.co/datasets/lmsys/lmsys-chat-1m?conversation-viewer=19
    assert isinstance(estimate_text_tokens(message2, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message2, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(5, abs=2)

    message3 = """
    for op in operations:
        if op.type == "SELECT":
    """ # http://huggingface.co/datasets/lmsys/lmsys-chat-1m?conversation-viewer=19
    assert isinstance(estimate_text_tokens(message3, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message3, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(21, abs=9)
...
def test_universal_code_with_variable_names():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(9, abs=3)


def test_universal_text_empty_and_whitespace():
    message1 = ""
    assert isinstance(estimate_text_tokens(message1, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message1, TextEstimator.RULE_BASED_UNIVERSAL) == 0

    message2 = " \t \n "
    assert isinstance(estimate_text_tokens(message2, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message2, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(5, abs=25)
...
def test_universal_text_non_english_with_special_chars():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(7, abs=2)
...
def test_universal_text_non_english():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_UNIVERSAL) == pytest.approx(31, abs=19)
...
def test_openai_GPT_3_5_text_with_simple_prompt():
    ...
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(6, abs=6)
...
def test_openai_GPT_3_5_text_with_punctuation():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(10, abs=8)
...
def test_openai_GPT_3_5_text_with_code_keywords():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(10, abs=7)


def test_openai_GPT_3_5_text_with_long_word():
    message = "This is a verylongwordwithoutspaces and should be counted properly."
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(15, abs=11)
...
def test_openai_GPT_3_5_text_with_url():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(31, abs=4)
...
def test_openai_GPT_3_5_text_with_rare_character():
    ...
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(18, abs=4)


def test_openai_GPT_3_5_text_with_newlines():
    message = "Line1\nLine2\nLine3"
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(8, abs=3)
...
def test_openai_GPT_3_5_text_with_non_english():
    message = "如何在sd上无错误进行模型训练"  # https://huggingface.co/datasets/allenai/WildChat-1M?conversation-viewer=20
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_3_5) == pytest.approx(31, abs=19)


def test_openai_gpt_4_text_with_non_english():
    message = "如何在sd上无错误进行模型训练"  # https://huggingface.co/datasets/allenai/WildChat-1M?conversation-viewer=20
    assert isinstance(estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_4), int)
    assert estimate_text_tokens(message, TextEstimator.RULE_BASED_GPT_4) == pytest.approx(31, abs=19)


def test_raises_error_for_invalid_text():
    invalid_text = 12345
    with pytest.raises(ValueError, match=INVALID_TEXT_MESSAGE): estimate_text_tokens(invalid_text)


def test_raises_error_for_invalid_estimator():
    valid_text = "sample prompt"
    invalid_estimator = "not a valid estimator"
    with pytest.raises(ValueError, match=INVALID_TEXT_ESTIMATOR_MESSAGE): estimate_text_tokens(valid_text, invalid_estimator)
